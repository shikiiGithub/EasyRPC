...
namespace LitJson
{
    ...
    public class JsonMapper
    {
        ...
        private static object ReadValue (Type inst_type, JsonReader reader)
        {
            ...
            if (reader.Token == JsonToken.Double ||
                ...
                if (value_type.IsAssignableFrom (json_type))
                    return reader.Value;
                if(value_type.Name=="Color")
                {
                    MethodInfo[] mifs = value_type.GetMethods(BindingFlags.Public | BindingFlags.Static);
                    int index = mifs.ToList().FindIndex(x => x.Name== "FromArgb" && x.GetParameters().Length == 1);
                    if(index !=-1)
                    {
                        Object obj = mifs[index].Invoke(null, new object[] { reader.Value });
                        return obj;
                    }
            ...
            if (reader.Token == JsonToken.ArrayStart) {
                ...
                instance = Activator.CreateInstance (value_type);
                 //  = instance.GetType().GetCustomAttributes().ToList().Find(x => { });
                 //todo 自定义Json反序列化
                while (true) {
                    reader.Read ();
                                         ...
                                         int depth,bool IgnoreField=false )
        {
            ...
            {
                writer.Write((int)obj.GetType().GetMethod("ToArgb").Invoke(obj,null));
                return;
            }
            ...
            foreach (PropertyMetadata p_data in props)
            {
                ...
                else
                {
                    ...
                    //Attribute attr  = p_info.GetCustomAttribute<IgnorePropAttribute>(true);
                    //if (attr != null)
                    //    continue;
                    if (p_info.CanRead)
                    {
        ...
        public static void ToObject(string json, System.Dynamic.ExpandoObject dyn_Object)
        {
            ...
            JsonData data = (JsonData)ToWrapper(
                 delegate { return new JsonData(); }, json);

            IDictionary<String, Object> pairs = dyn_Object;

            IEnumerator<String> iter = pairs.Keys.GetEnumerator();
            ...
            {
                JsonData obj = data[Keys[i]];

                if (obj.IsString)
                    tempObject = (String)obj;
                if (obj.IsInt)
                    tempObject = (int)obj;
                if (obj.IsDouble)
                    tempObject = (double)obj;
                if (obj.IsLong)
                    tempObject = (long)obj;
                if (obj.IsBoolean)
                    tempObject = (bool)obj;
                ...
                {
                    tempObject = obj;
                    tempObject = (Array)tempObject;
                }
                pairs[Keys[i]] = tempObject;
